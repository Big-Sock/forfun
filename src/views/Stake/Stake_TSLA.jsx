...
const Stake_TSLA = () => {
  ...
  const [onPresentStake] = useModal(
    <StakeModal max={balance} onConfirm={stake} tokenName={"wTSLA"} />
  );

  const [onPresentUnstake] = useModal(
    <UnstakeModal max={currStaked} onConfirm={unstake} tokenName={"wTSLA"} />
  );
  ...
  return (
      <Container>
        <PoolContainer>
          <PoolTitle>wTSLA</PoolTitle>
          <PoolInfo>
            <Line>
              Your Balance:
              <ShadedLine>
                {parseFloat(balance).toFixed(2)} wTSLA
              </ShadedLine>
            </Line>
            <Line>
              Currently Staked:
