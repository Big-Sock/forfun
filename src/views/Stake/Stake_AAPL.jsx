...
const Stake_AAPL = () => {
  ...
  const [onPresentStake] = useModal(
    <StakeModal max={balance} onConfirm={stake} tokenName={"wAAPL"} />
  );

  const [onPresentUnstake] = useModal(
    <UnstakeModal max={currStaked} onConfirm={unstake} tokenName={"wAAPL"} />
  );
  ...
  return (
    <Container>
      <PoolContainer>
        <PoolTitle>wAAPL</PoolTitle>
        <PoolInfo>
          <Line>
            Your Balance:
              <ShadedLine>
              {parseFloat(balance).toFixed(2)} wAAPL
              </ShadedLine>
          </Line>
          <Line>
            Currently Staked:
